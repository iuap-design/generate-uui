...
(function($, window, document, undefined) {
	...
	gridComp.prototype = {
		...
		createDivs: function() {
			...
			// this.ele.insertAdjacentHTML('afterBegin', htmlStr);
			window.top.document.body.appendChild($(htmlStr)[0]);
...
//				}
		...
		$('#' + this.options.id + '_top',$(window.top.document)).on('mousemove', function(e) {
			oThis.dragFun(e);
			e.stopPropagation();
		});
	...
	gridCompProto.dragFun = function(e) {
		if (this.dragFlag) {
			...
			$('#' + this.options.id + '_top',$(window.top.document)).css('display', 'block');
		}
	};
	/*
	 * 拖动结束
	...
	gridCompProto.dragEnd = function(e) {
		...
		this.dragFlag = false;
		$('#' + this.options.id + '_top',$(window.top.document)).css('display', 'none');
	};
	if(typeof gridCompProto.saveGridCompColumnArrToLocal == 'undefined'){
		gridCompProto.saveGridCompColumnArrToLocal = function(){
		};
